...
class _EditStationScreenState extends State<EditStationScreen> {
    ...
    if (_fuelType == 'EV') {
      ...
      if (chargingPoints != null && chargingPoints is List) {
        _chargingPoints = List<Map<String, dynamic>>.from(chargingPoints.map((c) {
          ...
          final isCustom = connector.isNotEmpty && !standardConnectors.contains(connector);
          return {
            'connector': connector,
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      // EV DATA 
      if (_fuelType == 'EV') {
        final chargingPointsData = _chargingPoints.map((c) {
          final connector = c['connector'] ?? '';
          return {
            'connector': connector.isNotEmpty ? connector : 'Unknown',
        ...
        updatedData['charging_points'] = chargingPointsData;
        updatedData['has_backup_generator'] = _hasBackupGenerator;

        print('=== SENDING CHARGING POINTS ===');
        print(chargingPointsData);
      }
