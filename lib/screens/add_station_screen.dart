...
class _AddStationScreenState extends State<AddStationScreen> {
  ...
  //  EV CONTROLLER LISTS (PERSISTENT) 
  final List<TextEditingController> _connectorControllers = [];
  ...
  final List<Map<String, dynamic>> _petrolOctanes = [
  ...
  final List<Map<String, dynamic>> _dieselTypes = [
  ...
  final List<Map<String, dynamic>> _chargingPoints = [
    {'connector': 'CCS', 'power': '150', 'price': '', 'available': true, 'isCustom': false},
  ];
  ...
  void _initControllers() {
    _connectorControllers.clear();
    _powerControllers.clear();
    _priceControllers.clear();

    for (int i = 0; i < _chargingPoints.length; i++) {
      _connectorControllers.add(TextEditingController(
        text: _chargingPoints[i]['connector'] ?? '',
      ));
      _powerControllers.add(TextEditingController(
        text: _chargingPoints[i]['power']?.toString() ?? '',
      ));
      _priceControllers.add(TextEditingController(
        text: _chargingPoints[i]['price']?.toString() ?? '',
      ));
    }
  }
  ...
  void _removeChargingPoint(int index) {
    setState(() {
      _connectorControllers[index].dispose();
      _connectorControllers.removeAt(index);
      _powerControllers[index].dispose();
      _powerControllers.removeAt(index);
  ...
  // Helper method for connector dropdown
  String? _getConnectorValue(Map<String, dynamic> point) {
    final connector = point['connector'];
    final isCustom = point['isCustom'] == true;

    if (isCustom) {
      return 'Other';
    }

    const standardConnectors = ['CCS', 'Type 2', 'CHAdeMO', 'Tesla', 'Type 1', 'GB/T'];
    if (standardConnectors.contains(connector)) {
      return connector;
    }
    ...
    try {
      ...
      if (_selectedFuelType == 'Petrol/Diesel') {
        ...
        newStation['ev_data'] = _chargingPoints.map((c) => ({
          ...
          'power_kw': int.tryParse(c['power']?.toString() ?? '0') ?? 0,
          'price_per_kwh': double.tryParse(c['price']?.toString() ?? '0') ?? 0,
          'available': c['available'] ?? true,
        })).toList();
        newStation['has_backup_generator'] = _hasBackupGenerator;

        // Fallback price
        if (_chargingPoints.isNotEmpty && _chargingPoints[0]['price']?.isNotEmpty == true) {
                    ...
                    return Column(
                      children: [
                        RadioListTile<String>(
                          title: Text(type, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500)),
                          value: type,
              ...
              if (_selectedFuelType == 'Petrol/Diesel') ...[
                ...
                Row(
                  children: [
                    ...
                    const Text('Sells Petrol', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ],
                ),
                ...
                Row(
                  children: [
                    ...
                    const Text('Sells Diesel', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ],
                ),
                                ...
                                children: [
                                  DropdownButtonFormField<String>(
                                    ...
                                    onChanged: (val) {
                                      setState(() {
                                        if (val == 'Other') {
                                          _chargingPoints[index]['connector'] = '';
                                          _chargingPoints[index]['isCustom'] = true;
                                          _connectorControllers[index].text = '';
                                        } else {
                                          _chargingPoints[index]['connector'] = val;
                                          _chargingPoints[index]['isCustom'] = false;
                                          _connectorControllers[index].text = val ?? '';
                                        }
                                      });
                                    },
                                  ),
                                  if (_chargingPoints[index]['isCustom'] == true)
                                    Padding(
                                      padding: const EdgeInsets.only(top: 8),
                                      child: TextFormField(
                                        controller: _connectorControllers[index],
                                        textDirection: TextDirection.ltr,
                                        decoration: const InputDecoration(
                                          labelText: 'Enter Custom Connector',
                                          hintText: 'e.g. NACS, Mennekes, etc.',
                                          border: OutlineInputBorder(),
                            ...
                            SizedBox(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const Flexible(
                                    child: Text('Avail', style: TextStyle(fontSize: 11)),
                                  ),
                                ],
                              ),
                            ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isSubmitting || _isUploading
                      ...
                      : const Text('Submit Station', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ),
              ),
